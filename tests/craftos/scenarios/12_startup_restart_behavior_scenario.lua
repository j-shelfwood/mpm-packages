return function(h)
    ...
    h:test("startup behavior: generated startup.lua executes full boot sequence despite command failures", function()
        ...
        local ok, err = run_mpm("startup", "shelfos", "headless")
        h:assert_true(ok, "mpm startup shelfos headless failed: " .. tostring(err))
        ...
        h:assert_contains(startupContent, "shell.run('mpm run shelfos headless')", "startup.lua missing configured run command")
        ...
        h:assert_eq("shelfos", config.package, "startup.config package mismatch")
        h:assert_eq("headless", config.parameters, "startup.config parameters mismatch")
        ...
        h:assert_eq("mpm run shelfos headless", calls[3], "Unexpected third startup command")
    end)
    ...
    h:test("startup behavior: refresh regenerates corrupted startup.lua", function()
        ...
        h:assert_contains(refreshed, "mpm run shelfos", "Refreshed startup.lua should include configured run target")
    end)
end
