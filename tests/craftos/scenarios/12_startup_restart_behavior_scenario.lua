return function(h)
    ...
    h:test("startup behavior: generated startup.lua executes full boot sequence despite command failures", function()
        ...
        h:assert_contains(startupContent, "shell.run('mpm run shelfos')", "startup.lua missing configured run command")
        ...
        h:assert_eq("mpm run shelfos", calls[3], "Unexpected third startup command")
    end)
