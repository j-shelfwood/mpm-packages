...
# Runs Lua unit tests, CraftOS-PC integration tests, and swarm simulation tests
...
if [ -x "$CRAFTOS" ]; then
    ...
    # =============================================================================
    # CRAFTOS-PC SWARM SIMULATION TESTS
    # =============================================================================

    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${YELLOW}  Running CraftOS-PC Swarm Simulation Tests (Multi-Computer)${NC}"
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""

    SWARM_OUTPUT=$(timeout 120 "$CRAFTOS" --headless \
        --mount-ro /workspace="$MPM_PACKAGES" \
        --exec "dofile('/workspace/tests/craftos/swarm_simulation.lua')" 2>&1 || true)

    # Extract clean results
    SWARM_RESULTS=$(echo "$SWARM_OUTPUT" | grep -E "^\[PASS\]|\[FAIL\]|^Passed:|^ALL|^TESTS" | uniq | tail -20)

    SWARM_PASSED=$(echo "$SWARM_RESULTS" | grep -c "^\[PASS\]" || true)
    SWARM_FAILED=$(echo "$SWARM_RESULTS" | grep -c "^\[FAIL\]" || true)

    # Show results
    echo "$SWARM_RESULTS" | while read -r line; do
        if [[ "$line" == "[PASS]"* ]]; then
            echo -e "${GREEN}$line${NC}"
        elif [[ "$line" == "[FAIL]"* ]]; then
            echo -e "${RED}$line${NC}"
    ...
    TOTAL_PASSED=$((TOTAL_PASSED + SWARM_PASSED))
    TOTAL_FAILED=$((TOTAL_FAILED + SWARM_FAILED))

else
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${YELLOW}  CraftOS-PC Tests (SKIPPED)${NC}"
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
