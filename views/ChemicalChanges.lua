...
-- Compares current chemicals against a baseline taken at period start
...
-- Take a snapshot of current chemical amounts
-- Returns: table {[name] = amount}, chemicalCount, success
local function takeSnapshot(interface)
    ...
    if not interface:hasChemicalSupport() then
        return {}, 0, false
    end

    local ok, chemicals = pcall(function() return interface:chemicals() end)
    if not ok or not chemicals or #chemicals == 0 then
        return {}, 0, false
    end
    ...
    for _, chemical in ipairs(chemicals) do
        if chemical.name then
            local amount = chemical.count or chemical.amount or 0
            if amount > 0 then
                snapshot[chemical.name] = (snapshot[chemical.name] or 0) + amount
                count = count + 1
            end
        end
        -- Yield periodically
...
local function calculateTotals(changes)
    ...
    for _, chemical in ipairs(changes) do
        if chemical.change > 0 then
            gains = gains + chemical.change
        else
            losses = losses + math.abs(chemical.change)
        end
    end
    return gains, losses
end
...
local function formatChange(chemical)
    local color = chemical.change > 0 and colors.lime or colors.red
    local sign = chemical.change > 0 and "+" or ""
    local buckets = chemical.change / 1000
    return {
        lines = { Text.prettifyName(chemical.id), sign .. Text.formatNumber(buckets, 1) .. "B" },
        colors = { colors.white, color }
    }
end

-- Show chemical detail overlay (blocking)
local function showChemicalDetail(self, chemical)
    ...
    while true do
        ...
        -- Title bar
        local titleColor = chemical.change > 0 and colors.lime or colors.red
        ...
        -- Chemical name
        local chemicalName = Text.prettifyName(chemical.id)
        ...
        monitor.setCursorPos(x1 + 1, contentY)
        monitor.write(Core.truncate(chemical.id, overlayWidth - 2))
        contentY = contentY + 2
        ...
        monitor.write(Text.formatNumber(chemical.baseline / 1000, 1) .. "B")
        monitor.setTextColor(colors.gray)
        monitor.write(" -> ")
        monitor.setTextColor(colors.lightBlue)
        monitor.write(Text.formatNumber(chemical.current / 1000, 1) .. "B")
...
return BaseView.custom({
    ...
    getData = function(self)
        ...
        if not self.interface:hasChemicalSupport() then
            return { error = "Applied Mekanistics not loaded" }
        end
        ...
        local current, chemicalCount, ok = takeSnapshot(self.interface)
        if not ok then
            return { error = "Error reading chemicals" }
        end
    ...
    render = function(self, data)
        ...
        if data.status == "waiting" then
            MonitorHelpers.writeCentered(self.monitor, math.floor(self.height / 2) - 1, "Chemical Changes", colors.white)
        ...
        if data.status == "baseline_captured" then
            MonitorHelpers.writeCentered(self.monitor, math.floor(self.height / 2) - 1, "Chemical Changes", colors.white)
            MonitorHelpers.writeCentered(self.monitor, math.floor(self.height / 2) + 1, "Baseline: " .. data.baselineCount .. " chemicals", colors.lime)
        ...
        if data.status == "period_reset" then
            ...
            MonitorHelpers.writeCentered(self.monitor, math.floor(self.height / 2) + 1, "New baseline: " .. data.baselineCount .. " chemicals", colors.gray)
            drawTimerBar(self.monitor, self.height, self.width, 0, self.periodSeconds)
            return
        end
        ...
        -- No changes
        if #changes == 0 then
            MonitorHelpers.writeCentered(self.monitor, 1, "Chemical Changes", colors.lightBlue)
    ...
    onTouch = function(self, x, y)
        ...
        if y >= startY and y < self.height then
            ...
            if index >= 1 and index <= #self.lastChanges then
                showChemicalDetail(self, self.lastChanges[index])
                return true
            end
        end
    ...
    errorMessage = "Error tracking chemical changes"
})
