...
local function getFlowColor(netFlow, warningThreshold)
    if netFlow >= 0 then
        return colors.lime
...
local function getStorageColor(percent)
    if percent <= 20 then
        return colors.red
...
local function renderCompact(self, data, flowColor, storageColor)
    ...
    if self.height >= 4 then
        ...
        if data.timeToFull then
            self.monitor.write("Full: " .. formatDuration(data.timeToFull))
        elseif data.timeToEmpty then
            self.monitor.write("Empty: " .. formatDuration(data.timeToEmpty))
        else
...
local function renderDetailed(self, data, flowColor, storageColor)
    local y = 1
    ...
    if self.height >= y then
        ...
        self.monitor.setCursorPos(1, y)
        if data.timeToFull then
            self.monitor.write("Full in " .. formatDuration(data.timeToFull))
        elseif data.timeToEmpty then
            self.monitor.write("Empty in " .. formatDuration(data.timeToEmpty))
        else
            self.monitor.write("Stable")
        end
    end
...
local function renderWithGraph(self, data, flowColor, storageColor)
    -- Row 1: Title + percentage
    self.monitor.setTextColor(colors.cyan)
    self.monitor.setCursorPos(1, 1)
    self.monitor.write("ENERGY STATUS")
    ...
    if self.height >= 9 then
        ...
        for i, value in ipairs(self.history) do
            if i <= self.width then
                local barColor = getFlowColor(value, warningThreshold)
...
return BaseView.custom({
    ...
    render = function(self, data)
        local flowColor = getFlowColor(data.netFlow, self.warningThreshold)
        local storageColor = getStorageColor(data.percentage)

        if self.displayMode == "compact" then
            renderCompact(self, data, flowColor, storageColor)
        elseif self.displayMode == "graph" then
            renderWithGraph(self, data, flowColor, storageColor)
        else
            renderDetailed(self, data, flowColor, storageColor)
        end
    end,
