...
return BaseView.custom({
    ...
    render = function(self, data)
        ...
        if self.displayMode == "compact" then
            self:renderCompact(data, flowColor, storageColor)
        elseif self.displayMode == "graph" then
            self:renderWithGraph(data, flowColor, storageColor)
        else
            self:renderDetailed(data, flowColor, storageColor)
        end
    end,

    -- Compact mode: minimal footprint for small monitors
    renderCompact = function(self, data, flowColor, storageColor)
    ...
    renderDetailed = function(self, data, flowColor, storageColor)
        local y = 1
    ...
    renderWithGraph = function(self, data, flowColor, storageColor)
        -- Row 1: Title + percentage
        self.monitor.setTextColor(colors.cyan)
        self.monitor.setCursorPos(1, 1)
        self.monitor.write("ENERGY STATUS")
